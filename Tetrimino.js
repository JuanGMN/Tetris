class Tetrimino {
  ...
  get estaDentroDelTablero() {
    for (const pmino of this.mapaTablero) {
      if (pmino.x < 0) {
        //Evita la salida por el lado izquierdo
        return false;
      }
      if (pmino.x >= tablero.columnas) {
        //Evita la salida por el lado Derecho
        return false;
      }
      if (pmino.y >= tablero.filas) {
        //Evita la salida por Abajo
        return false;
      }
    }
    return true;
  ...
  /* Esta funcion de encargara del procesamiento logico del dibujado del Tetrimino */
  dibujar() {
    push();
    fill(this.color);
  ...
  static dibujarMino(pmino){
    ...
    noStroke();
    fill(255, 255, 255, 80);
    //Dibuja el efecto de cristal que tienen los minos
    beginShape();
...
function crearMapeoBaseTetriminos() {
  //Muy importante, no ponerle let, var, ni const de prefijo
  tetriminosBase = {
    Z: {
      color: "red",
      mapa: [
